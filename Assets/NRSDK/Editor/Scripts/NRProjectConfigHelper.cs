...
namespace NRKernal
{
    ...
    public static class NRProjectConfigHelper
    {
        ...
        public static void CommitProjectConfig(NRProjectConfig projectConfig)
        {
            ...
            ApplyEnableNativeSessionManagerConfig(projectConfig);
        }
        ...
        private static void ApplyEnableNativeSessionManagerConfig(NRProjectConfig projectConfig)
        {
            var defines = PlayerSettings.GetScriptingDefineSymbolsForGroup(BuildTargetGroup.Android);
            var symbol = NRProjectConfig.ENABLE_NATIVE_SESSION_MANAGER;
#if !ENABLE_NATIVE_SESSION_MANAGER
            if (projectConfig.enableNativeSessionManager)
            {
