...
namespace NRKernal
{
    ...
    internal partial class NativeSwapchain
    {
        ...
        internal void SubmitFrame(UInt64 frameHandle)
        {
            ...
            DateTime timeBeforeSubmit = DateTime.MinValue;
            if (NRDebugger.logLevel <= LogLevel.Debug)
            {
                timeBeforeSubmit = DateTime.Now;
            ...
            if (NRDebugger.logLevel <= LogLevel.Debug)
            {
                var duration = (DateTime.Now - timeBeforeSubmit).Milliseconds;
                NRDebugger.Info("[NativeSwapchain] SubmitFrame after wait: {0}ms, {1}", duration, NRFrame.CurrentPoseTimeStamp);
            }
